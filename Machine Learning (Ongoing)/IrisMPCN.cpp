...
#include <cstddef>
#include <cmath>
#include "C:\Code\C++\ML\Eigen\Dense"
#include <vector>
#include <functional>
#include <memory>
...
class Pcn2L {
	public :
		...
		oMatrix& eval(iMatrix& inMx) {
        //MEMORY :: maybe leak
			oMatrix* out {new oMatrix(inMx.size())};
		...
		void train(iVect& in, oVect& target) {
			...
			std::unique_ptr<std::vector<double>> deriv_l1
						{ new std::vector<double>(target.size())};
			...
			/**Updating Layer 0 */
			std::unique_ptr<std::vector<double>> deriv_l0
						{ new std::vector<double>(hidden->size())};
		...
		double error(iMatrix& inMx, oMatrix& target) {
			...
			for(int i = 0; i < inMx.size(); i++) {
				...
				//MEMORY :: maybe leak?
				//does std::shared_ptr free buffer on being reasigned a value?
...
int main()
{
	...
	FILE* iris = fopen("C:\\Code\\C++\\ML\\iris_edited.data","r");
	assert(iris);
	for(int i = 0; i < 150; i++) {
